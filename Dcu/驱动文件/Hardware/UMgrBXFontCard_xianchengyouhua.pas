...
begin
  ...
  begin
    FCards[i].FClient                := TIdTCPClient.Create(nil); 
    FCards[i].FClient.ReadTimeout    := 3 * 1000;
    FCards[i].FClient.ConnectTimeout := 3 * 1000;
    FCards[i].FClient.Host           := FCards[i].FHost;
    FCards[i].FClient.Port           := FCards[i].FPort;
  end;
...
begin
  ...
  for i := Low(FCards) to High(FCards) do
  begin
    FCards[i].FClient.Disconnect;
    FCards[i].FClient.Free;
  end;
end;
...
var nStr: string;
  ...
  //Desc: 向屏卡发送数据
  procedure SendBuffer(const nClient: TIdTCPClient);
  begin
...
//      DisconnectClient;
  ...
  //        nClient.IOHandler.InputBuffer.Clear;
        nClient.Connect;
        nClient.Socket.Write(nBuf);
        //send
      except
        on nErr: Exception do
        begin
          nStr := '屏卡[ %s.%d ]发送错误,描述: %s';
          nStr := Format(nStr, [nClient.Host, nClient.Port, nErr.Message]);
          WriteLog(nStr);
        end;
      end;
  finally
