...
begin
  ...
  begin
    ...
    nIdx := FieldByName('C_Type').AsInteger;
    if nIdx > 2 then
      nIdx := 0;
    EditType.ItemIndex := nIdx;
...
begin
  ...
  if not IsNumber(Trim(EditValue.Text), True) then
  begin
    ActiveControl := EditValue;
    ShowMsg('请输入有效吨位上限', sHint);
    Exit;
  end;
  nVal := StrToFloat(Trim(EditValue.Text));
