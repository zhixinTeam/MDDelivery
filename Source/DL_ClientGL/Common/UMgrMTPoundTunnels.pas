...
type
  ...
  TOnTunnelDataEvent = procedure (const nValue: string) of object;
...
var
  gMTPoundTunnelManager: TPoundTunnelManager = nil;
  //全局使用
...
begin
  gSysLoger.AddLog(TPoundTunnelManager, '抓斗称磅站通道管理', nEvent);
end;
...
begin
  ...
  begin
    ...
    for i:=1 to Length(nPort.FCOMData) do
    begin
      if Trim(nPort.FCOMData[i]) <> '' then
      nStr := nStr + nPort.FCOMData[i];
    end;
    nPort.FCOMData := nStr;

    WriteLog('通讯数据:' + nPort.FCOMData);
...
begin
  ...
  begin
    ...
    FActiveTunnel.FOnData(FActivePort.FCOMData);
...
initialization
  gMTPoundTunnelManager := nil;
finalization
  FreeAndNil(gMTPoundTunnelManager);
end.
