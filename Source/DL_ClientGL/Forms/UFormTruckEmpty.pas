...
uses
  ...
  USysBusiness, USysDB, USysConst, UFormBase, UDataModule;
...
var nRet: Boolean;
    nNext, nStr: String;
begin
  nStr := ' Select isnull(L_MValue,0)-isnull(L_PValue,0) as L_NetNeight From S_Bill Where L_ID = ''%s'' ';
  nStr := Format(nStr, [gBills[0].FID]);
  FDM.QueryTemp(nStr);
  with FDM.SqlTemp do
  begin
    if RecordCount > 0 then
    begin
      if FieldByName('L_NetNeight').AsFloat > 0.5 then
      begin
        ShowMsg('重车不允许办理空车出厂', sHint);
        Exit;
      end;
    end;
  end;
