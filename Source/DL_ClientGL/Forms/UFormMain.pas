...
type
  ...
  private
    ...
    FZTHandle,FZTIndex: Integer;
    FTHHandle,FTHIndex: Integer;
    FTrayIcon: TTrayIcon;
    {*状态栏图标*}
  protected
...
uses
  ...
  UFormWait, UFormLogin, UFormBase,UFrameZTDispatch,UFrameBillRefresh;
...
begin
  ...
  begin
    ...
    if nSheet.Hint = 'MAIN_F04' then
    begin
      FZTIndex  := nSheet.TabIndex+1;
      FZTHandle := TBaseFrame(nCtrl).Handle;
      PostMessage(FZTHandle,WM_FrameActive,2,0);
      PostMessage(FTHHandle,WM_FrameTHActive,1,0);
    end
    else if nSheet.Hint = 'MAIN_D16' then
    begin
      FTHIndex  := nSheet.TabIndex+1;
      FTHHandle := TBaseFrame(nCtrl).Handle;
      PostMessage(FTHHandle,WM_FrameTHActive,2,0);
      PostMessage(FZTHandle,WM_FrameActive,1,0);
    end else
    begin
      PostMessage(FZTHandle,WM_FrameActive,1,0);
      PostMessage(FTHHandle,WM_FrameTHActive,1,0);
...
begin
  if wPage.PageCount > 1 then
  begin
    if FZTIndex = wPage.ActivePage.PageIndex then
    begin
      PostMessage(FZTHandle,WM_FrameActive,2,0);
    end
    else
    ...
    if FTHIndex = wPage.ActivePage.PageIndex then
    begin
      PostMessage(FTHHandle,WM_FrameTHActive,2,0);
    end
    else
