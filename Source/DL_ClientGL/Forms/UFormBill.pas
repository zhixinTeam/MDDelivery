...
begin
  ...
  begin
    nStr := Format('纸卡[ %s ]没有可提的骨料品种,已终止.', [gInfo.FZhiKa]);
    ShowDlg(nStr, sHint);
...
begin
  ...
  nStr := Format(nStr, [sTable_SysDict, 'PaymentItem', gInfo.FZhiKa]);
  with FDM.QueryTemp(nStr) do
  if RecordCount > 0 then
  begin
