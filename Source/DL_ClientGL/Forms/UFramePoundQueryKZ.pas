...
type
  TfFramePoundQueryKZ = class(TfFrameNormal)
...
class function TfFramePoundQueryKZ.FrameID: integer;
begin
  Result := cFI_FramePoundQueryKZ;
end;

procedure TfFramePoundQueryKZ.OnCreateFrame;
begin
  inherited;
...
procedure TfFramePoundQueryKZ.OnDestroyFrame;
begin
...
function TfFramePoundQueryKZ.GetVal(const nRow: Integer;
...
function TfFramePoundQueryKZ.InitFormDataSQL(const nWhere: string): string;
begin
  FEnableBackDB := True;
  //启用备份数据库
  ...
  begin
    Result := Result + ' Where ((P_PDate >=''$S'' and P_PDate<''$E'') or ' +
              '(P_MDate >=''$S'' and P_MDate<''$E'')) and P_Type = ''P'' ';
  end else
  begin
    Result := Result + ' Where (' + FJBWhere + ') and P_Type = ''P'' ';
  end;
...
procedure TfFramePoundQueryKZ.AfterInitFormData;
begin
  FJBWhere := '';
end;

//Desc: 日期筛选
procedure TfFramePoundQueryKZ.EditDatePropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
//Desc: 执行查询
procedure TfFramePoundQueryKZ.EditTruckPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
procedure TfFramePoundQueryKZ.CheckDeleteClick(Sender: TObject);
begin
  BtnRefresh.Click;
end;
...
//------------------------------------------------------------------------------
//Desc: 权限控制
procedure TfFramePoundQueryKZ.PMenu1Popup(Sender: TObject);
begin
  //
end;

//Desc: 打印磅单
procedure TfFramePoundQueryKZ.N3Click(Sender: TObject);
var nStr: string;
begin
...
//Desc: 时间段查询
procedure TfFramePoundQueryKZ.N2Click(Sender: TObject);
begin
...
//Desc: 删除榜单
procedure TfFramePoundQueryKZ.BtnDelClick(Sender: TObject);
var nIdx: Integer;
    nStr,nID,nP: string;
...
//Desc: 查看抓拍
procedure TfFramePoundQueryKZ.N4Click(Sender: TObject);
...
procedure TfFramePoundQueryKZ.N6Click(Sender: TObject);
var nStr: string;
    nIdx: Integer;
    nList: TStrings;
begin
...
procedure TfFramePoundQueryKZ.cxView1DblClick(Sender: TObject);
var nStr: string;
...
procedure TfFramePoundQueryKZ.N10Click(Sender: TObject);
var
  nID, nMan : string;
  nList : TStrings;
  nP: TFormCommandParam;
begin
  ...
  nID := SQLQuery.FieldByName('P_ID').AsString;
  nMan:= SQLQuery.FieldByName('P_AdjustMan').AsString;
  if Length(nMan) > 0 then
  begin
    ShowMsg('此记录已经二次扣重,不能再修改', sHint);
    Exit;
  end;
...
initialization
  gControlManager.RegCtrl(TfFramePoundQueryKZ, TfFramePoundQueryKZ.FrameID);
end.
