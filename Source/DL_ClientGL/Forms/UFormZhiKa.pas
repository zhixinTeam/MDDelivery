...
type
  ...
  protected
    ...
    //载入数据
    function IsRepeatZhiKaID(const nCusID,nPayment,nZKName:string):Boolean;
...
begin
  ...
  FItemIndex := -1;
  AdjustCtrlData(Self);
  EditName.Properties.IncrementalSearch := False;
end;
...
//  if EditName.Properties.Items.Count < 1 then
//  begin
//    EditName.Clear;
//    EditName.Text := '';
//    LoadSysDictItem(sFlag_PaymentItem, EditName.Properties.Items);
//  end;
...
begin
  ...
  begin
    ShowMsg('请输入纸卡名称.',sHint);
    Exit;
  end;
  if Trim(EditPayment.Text) = '' then
  begin
    ShowMsg('请输入付款方式.',sHint);
    Exit;
  end;

//  if Trim(EditName.Text) <> Trim(EditPayment.Text) then
//  begin
...
//  end;
  ...
  if FRecordID = '' then
  begin
    if IsRepeatZhiKaID(nCID,EditPayment.Text,EditName.Text) then
    begin
      EditName.SetFocus;
...
function TfFormZhiKa.IsRepeatZhiKaID(const nCusID, nPayment,nZKName: string): Boolean;
var
  nStr:string;
begin
  Result := False;
  nStr := ' Select Z_ID from %s where Z_Customer = ''%s'' and Z_Name = ''%s'' and isnull(Z_InValid,''N'') <> ''Y''  and Z_PayType '+
          ' in (Select distinct D_Index from Sys_Dict where D_Name= ''PaymentItem'' and D_Value = ''%s'')';
  nStr := Format(nStr,[sTable_ZhiKa,nCusID,nZKName,nPayment]);
  with fdm.QueryTemp(nStr) do
  begin
    if RecordCount>0 then
    begin
...
begin
  if EditCustom.Text <> '' then
  begin
    EditName.Properties.Items.Clear;
    EditName.Clear;
    
    nStr := ' Select distinct P_PayingUnit From %s Where P_CustID = ''%s'' ';
    nStr := Format(nStr, [sTable_PayingUnit, GetLeftStr('.', Trim(EditCustom.Text))]);
    ...
    begin
      ...
      while not Eof do
      begin
        EditName.Properties.Items.Add(FieldByName('P_PayingUnit').AsString);
        Next;
      end;
    end;
    if EditName.Properties.Items.Count = 1 then
    begin
      EditName.ItemIndex := 0;
    end;
  end;
end;
